...
Keeps the iPhone screen on using the Screen Wake Lock API (`navigator.wakeLock.request('screen')`). The lock is requested on mount and re-acquired on `visibilitychange` (iOS releases the lock whenever the page is hidden). A stale-sentinel guard ensures that release events from a superseded lock are ignored. When a release *is* current, the composable immediately re-requests to close the gap before the screen auto-locks. A 10-second `setInterval` acts as a safety net for silent drops. Everything is cleaned up on unmount.
...
| **Screen Wake Lock API with aggressive re-acquire** | The native Screen Wake Lock API keeps the screen on over HTTPS. iOS Safari can silently release the lock, so the composable re-requests immediately on the `release` event and polls every 10 seconds as a safety net. |

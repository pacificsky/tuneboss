...
.track-wipe--horizontal {
  animation: wipe-horizontal 900ms cubic-bezier(0.4, 0, 0.2, 1) forwards;
}
...
.track-wipe--vertical {
  animation: wipe-vertical 900ms cubic-bezier(0.4, 0, 0.2, 1) forwards;
}
...
.track-wipe--radial {
  animation: wipe-radial 1000ms cubic-bezier(0.4, 0, 0.2, 1) forwards;
}

@keyframes wipe-radial {
  0%   { clip-path: circle(0% at 50% 35%); opacity: 0.85; }
  50%  { clip-path: circle(150% at 50% 35%); opacity: 0.85; }
  70%  { clip-path: circle(150% at 50% 35%); opacity: 0.85; }
  100% { clip-path: circle(150% at 50% 35%); opacity: 0; }
}

/* 4. Diagonal wipe — angled edge sweeps top-left to bottom-right */
.track-wipe--diagonal {
  animation: wipe-diagonal 900ms cubic-bezier(0.4, 0, 0.2, 1) forwards;
}

@keyframes wipe-diagonal {
  0%   { clip-path: polygon(0 0, 0 0, 0 100%, 0 100%); }
  40%  { clip-path: polygon(0 0, 120% 0, 100% 100%, 0 100%); }
  55%  { clip-path: polygon(0 0, 120% 0, 100% 100%, 0 100%); }
